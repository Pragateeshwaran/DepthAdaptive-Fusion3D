...
def compute_map(predictions, targets, iou_threshold=0.5):
    ...
    for class_name in class_names:
        ...
        for img_idx, tgt in enumerate(targets):
            ...
            gt_labels = _to_cpu_detached_tensor(tgt.get('labels', torch.zeros((0,), dtype=torch.long))).long()
        ...
        for img_idx, pred in enumerate(predictions):
            ...
            pred_labels = _to_cpu_detached_tensor(pred.get('labels', torch.zeros((0,), dtype=torch.long))).long()
            ...
            for b_idx in range(cls_boxes.shape[0]):
                pred_records.append((img_idx, cls_scores[b_idx].item(), cls_boxes[b_idx]))
        ...
        for i, (img_idx, _, pred_box) in enumerate(pred_records):
            ...
            ious = bev_iou(pred_box.unsqueeze(0), gt_boxes).squeeze(0).cpu()
