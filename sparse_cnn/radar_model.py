...
class RadarFeatureExtractor(nn.Module):
    """
    Radar Feature Extractor with dual branches (features + confidence).
    ...
    def __init__(self, in_channels=5, feature_dim=128):
        ...
        self.encoder = spconv.SparseSequential(
            # Input: in_channels -> 32
            SubMConv3d(in_channels, 32, 3, padding=1, bias=False, indice_key='subm0'),
            nn.BatchNorm1d(32),
            ...
            nn.BatchNorm1d(64),
            nn.ReLU(inplace=True),
            SubMConv3d(64, 64, 3, padding=1, bias=False, indice_key='subm1'),
            nn.BatchNorm1d(64),
            nn.ReLU(inplace=True),
            
            # 64 -> 96 (reduce spatial)
            SparseConv3d(64, 96, 3, stride=2, padding=1, bias=False),
            nn.BatchNorm1d(96),
            nn.ReLU(inplace=True),
            SubMConv3d(96, 96, 3, padding=1, bias=False, indice_key='subm2'),
            nn.BatchNorm1d(96),
            nn.ReLU(inplace=True),
            
            # 96 -> 128 (reduce spatial)
            SparseConv3d(96, 128, 3, stride=2, padding=1, bias=False),
            nn.BatchNorm1d(128),
            nn.ReLU(inplace=True),
            ...
            SubMConv3d(128, 128, 3, padding=1, bias=False, indice_key='subm4'),
            nn.BatchNorm1d(128),
            nn.ReLU(inplace=True),
        )
        ...
        self.confidence_branch = spconv.SparseSequential(
            ...
            # 32 -> 16
            SubMConv3d(32, 16, 3, padding=1, bias=False, indice_key='conf3'),
            nn.BatchNorm1d(16),
            nn.ReLU(inplace=True),
            
            # 16 -> 1 (confidence score)
            SubMConv3d(16, 1, 1, bias=True, indice_key='conf4'),
